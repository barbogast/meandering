...
const getNextHeadPosition = (state, snake) => {
...
const snakeWillDie = (state, newHead) => state.blocks.some((pos) => pos.x === newHead.x && pos.y === newHead.y) || state.snakes.filter((snake) => snake.isAlive).some((snake) => snake.body.some((pos) => pos.x === newHead.x && pos.y === newHead.y));
const snakeWillEat = (state, newHead) => state.apples.some((pos) => pos.x === newHead.x && pos.y === newHead.y);
export const moveSnake = (state, snake) => {
  if (!snake.isAlive) {
    return;
  }
...
export const addApple = (state) => {
  ...
  while (true) {
    ...
    if (!state.blocks.some((pos) => pos.x === apple.x && pos.y === apple.y) && !state.snakes.some((snake) => snake.body.some((pos) => pos.x === apple.x && pos.y === apple.y)) && !state.apples.some((pos) => pos.x === apple.x && pos.y === apple.y)) {
      break;
    }
    counter += 1;
    if (counter > 1e3) {
...
const findNextApple = (state, snake) => {
...
const moveTowardsTarget = (state, snake) => {
  const isHorizontal = ["left", "right"].includes(snake.directions[0]);
  const currentHead = snake.body[snake.body.length - 1];
  const newHead = getNextHeadPosition(state, snake);
  if (!snake.nextTarget || currentHead.x === snake.nextTarget.x && currentHead.y === snake.nextTarget.y) {
...
export const controlAiSnake = (state, snake) => {
  if (!snake.isAlive) {
    return;
  }
  const isHorizontal = ["left", "right"].includes(snake.directions[0]);
